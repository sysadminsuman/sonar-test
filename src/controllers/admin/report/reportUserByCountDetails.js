 ...
 * @param next
 */
export const reportUserByCountDetails = async (req, res, next) => {
  try {
    const reqBody = req.query;
    let reqPrams = req.params;
    let getId = reqPrams.id;
    //console.log(getId);
    ...
    const allReportedUser = await adminService.reportService.getReportUserCountDetailsList(
      searchParams,
      getId,
    );
    ...
    if (totalReportedUser > 0) {
      ...
      const reportedUserDetails =
        await adminService.reportService.reportUserByCountDetailsListByPagination(
          offSet,
          noOfRecordsPerPage,
          searchParams,
          getId,
      ...
      if (reportedUserDetails) {
        for (const user of reportedUserDetails) {
          let reportuserlist = {
            ...
            reported_userId: user.postedby_userId,
            reportBy_userId: user.reportby_userId,
            reported_emailId: user.postedby_emailId,
