...
export const getUserLatestChatHistory = async (req, res, next) => {
  try {
    ...
    //console.log(params);
    const allChatConversations = await chatService.getTotalLatestChatCoversationByRoomIdAndTime(
      params,
    );
    ...
    if (parseInt(totalChatConversations) > 0) {
      ...
      const chatConversations = await chatService.getUserLatestChatHistoryByPagination(params);
