...
export const getUserChatroomsByPagination = async (userId, offSet, limit) => {
  ...
  ${lastmassagetimtbl}
  ${lastmassagetbl}
  WHERE (c.status = 'active' OR c.is_moderator_left = 1) AND cm.status = 'active' AND cm.user_id = ? 
  ORDER BY latest_message_time DESC LIMIT ?, ?`;

  const result = await executeQuery(query, [userId, userId, userId, userId, offSet, limit]);
  return result;
};
