...
export const getUserLatestChatroomsByPagination = async (userId, offSet, limit) => {
  let qparams = [userId, userId, userId, userId, offSet, limit];
  ...
  and c.create_date >= m.create_date
  and m.user_id = ?
  ...
  ${lastmassagetimtbl}
  ${lastmassagetbl}
  WHERE c.status = 'active' AND cm.status = 'active' AND cm.user_id = ? 
