...
export const getUserChatroomsByPagination = async (userId, offSet, limit, searchParams) => {
  ...
  c.city, c.country,c.create_date, c.passcode,countUnreadChat(c.id, ? ) as total_unread, getLatestMessageByRoomId(c.id)  as latest_message, checkRoomOwnerByMemberId(c.id, ?) as owner, countActiveMemberByRoomID(c.id)  as active_members, getLatestMessageTimeByRoomId(c.id) as latest_message_time FROM ${TABLES.CHATROOMS_TABLE} AS c INNER JOIN ${TABLES.CHATROOM_MEMBERS_TABLE} AS   cm ON c.id = cm.room_id ${whereQuery}  ORDER BY cm.last_activity_date DESC LIMIT ?, ?`;
