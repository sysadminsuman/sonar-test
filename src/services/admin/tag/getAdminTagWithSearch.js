...
export const getAdminTagWithSearch = async (searchParams) => {
  ...
  if (searchParams && searchParams.startDate && !searchParams.endDate) {
    whereQuery += ' AND DATE(last_create_date) >="' + startDate + '"';
  ...
  const query = `SELECT t.id, t.name, t.create_date, t.status, hct_user.last_create_date AS lastUseDate,
   hct_user.cnt AS userCount, COALESCE(crt.chatroomCount, 0) AS chatroomCount FROM ${TABLES.TAG_TABLE} AS t 
   LEFT JOIN( 
   ...
   LEFT JOIN( 
    SELECT COUNT(*) AS chatroomCount, ct.tag_id 
    FROM ${TABLES.CHATROOM_TAGS_TABLE} AS ct join ${TABLES.CHATROOMS_TABLE} as c on c.id=ct.room_id 
    where ct.status <> "deleted" and c.status <> "deleted" GROUP BY tag_id 
    ) AS crt ON t.id = crt.tag_id ${whereQuery}  ${selectUserQuery}`;
