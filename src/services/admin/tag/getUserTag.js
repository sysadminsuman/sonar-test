...
export const getUserTag = async (searchParams) => {
  ...
  hct_user.cnt AS userCount, COALESCE(crt.chatroomCount, 0) AS chatroomCount 
  ...
  LEFT JOIN( 
  SELECT COUNT(*) AS chatroomCount, ct.tag_id, ct.status
  ...
  GROUP BY ct.tag_id ) AS crt ON t.id = crt.tag_id ${joinUTables} ${whereQuery} 
  group by t.id ORDER BY chatroomCount DESC , t.id ASC `;
