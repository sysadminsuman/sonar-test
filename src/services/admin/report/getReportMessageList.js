...
export const getReportMessageList = async (searchParams = {}) => {
  let params = [];
  let whereQuery = " 1=1 ";
  ...
  const query = `SELECT m.group_name, m.message ,m.content_type, mr.type, u.name as reported_by , m.posted_by ,mr.updated_by, m.group_type , mr.create_date, mr.description, mr.conversation_id, mr.update_date,
   m.chatroom_id,mr.user_id as reportby_userId ,m.u_id as postedby_userId ,m.email as postedby_emailId ,u.email as reportby_emailId,m.url_meta,m.postedby_customer_id,u.customer_id as repotedby_customer_id 
	FROM ${TABLES.CONVERSATION_REPORTINGS_TABLE} AS mr ${memberstbl} ${crtbl}  WHERE ${whereQuery} ORDER BY mr.id DESC `;
