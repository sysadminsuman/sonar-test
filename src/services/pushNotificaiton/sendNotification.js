      ...
      while (androidTokens.length > 0) {
        ...
        Promise.all(
          android_chunk.map((androidtoken) => {
            const userId = devices.find((device) => device.device_token === androidtoken).user_id;
            const badgeNumber = badgeNumbers[userId] || 0;
...
export const sendBadgeCountNotification = async (
  userIds,
  notificationTemplateType,
  badgeSilentNumbers = {},
) => {
  if (userIds.length !== 0) {
    ...
    Promise.all(
      androidTokens.map((androidtoken) => {
        const userId = devices.find((device) => device.device_token === androidtoken).user_id;
        const badgeNumber = badgeSilentNumbers[userId] || 0;
        const androidPayload = {
          token: androidtoken,
          data: {
            badge: badgeNumber.toString(),
